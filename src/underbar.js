(function() {
  ...
  _.every = function(collection, iterator) { // TIP: Try re-using reduce() here.
  ...
  _.zip = function(...arg) {
    var transpose = function(twoDArray){
      return _.map(twoDArray[0], function(element,index){
        return _.map(twoDArray, function(subArray){
  ...
  _.flatten = function(nestedArray, result) {
    var hasSubArray = function(arrayToBeTested){
      return _.some(arrayToBeTested, function(element){return Array.isArray(element)});
    };
    var newArray = _.reduce(nestedArray,function(previousResult, currentElement){
      if (Array.isArray(currentElement)) previousResult = previousResult.concat(currentElement);
      else previousResult.push(currentElement);
      return previousResult;
    },[]); 
    if (hasSubArray(newArray)) return _.flatten(newArray, result);
  ...
  _.intersection = function(...args) {
    args = args.sort(function(a, b){ return a.length - b.length});
    var firstArg = args.shift();
    return _.filter(firstArg, function(elementInFirstArgs){
      return _.every(args, function(eachArg){
        return _.some(eachArg, function(elementInEachArg){
          return elementInEachArg === elementInFirstArgs;
        })
      })
    }) 
  };
  ...
  _.difference = function(...args) {
    ...
    return _.filter(firstArg, function(elementInFirstArgs){
      return _.every(args, function(eachArg){
        return _.every(eachArg, function(elementInEachArg){
          return elementInEachArg !== elementInFirstArgs;
        })
      })
    }) 
